          ...
          return (
            <Link to={`/job/${job.id}`}>
              <div
                ...
                key={job.id}
              >
                <span className={`text-[16px] font-normal  text-slate-500 ${darkMode ? "colorVeryDarkBlue" : "bg-white"}`}>
                  {job.contract} <span className={` ml-1 mr-1 ${darkMode ? "colorVeryDarkBlue" : "bg-white"}`}>.</span>{" "}
                  {job.postedAt}
                </span>
                <h1 className={`font-bolde my-2 ${darkMode ? "text-white" : "text-black"} ${darkMode ? "colorVeryDarkBlue" : "bg-white"}`}>{job.position}</h1>
                <h2 className={`text-[16px] font-normal  mb-10 text-slate-500 ${darkMode ? "colorVeryDarkBlue" : "bg-white"}`}>
                  {" "}
                  {job.company}
                </h2>
                <p className={`w-[286px] md:w-[270px] text-indigo-500 text-[14px] font-bold  -mb-3 ${darkMode ? "colorVeryDarkBlue" : "bg-white"}`}>
                  {job.location}
                </p>{" "}
                <div
